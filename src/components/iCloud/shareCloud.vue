...
<template>
  <div class="shareCloud">
    <el-collapse v-model="activeNames">
      <div
        @click="clickBlock({e: $event,clickTag:'shareCloud',list:blankRight,dataArr:shareCloud})"
        @contextmenu.prevent="showRightMenu({e:$event,clickTag: 'shareCloud',dataArr:shareCloud,list:blankRight})"
      >
        <el-collapse-item name="1" :disabled="true">
          ...
          <div class="cards slideInDown animated" :class="displayName==='行展示'?'row':'col'">
            <el-card
              v-for="(c,index) in shareCloud"
              :class="_isClick.shareCloud&&c.active?'addBorder':''"
              :key="index"
              shadow="hover"
            >
              <div
                @click.stop="activeCard({e:$event,item:c,i:index,clickTag:'shareCloud',dataArr:shareCloud})"
                @contextmenu.stop.prevent="fileRightMenu({e:$event,item:c,dataArr:shareCloud,clickTag:'shareCloud'})"
    ...
    <el-dialog
      ...
      <el-form
        ...
        :rules="rules1"
        label-width="100px"
        class="cloudForm1 fadeIn animated"
        ref="cloudForm1"
      >
        <el-form-item label="云端名称" prop="name">
          <el-input v-model="cloudForm1.name" size="small"></el-input>
          <el-switch v-model="cloudForm1.selectSource" active-text="新建资源" inactive-text="已有资源"></el-switch>
        </el-form-item>
        <el-form-item
          v-if="cloudForm1.selectSource"
          label="选择资源"
          prop="source"
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane :disabled="true">
            <span slot="label" class="tab-title">
              <img src="@/assets/image/icons/fileIcons/pepole.png" />权限维度
            </span>
          </el-tab-pane>
          <el-tab-pane label="内部组织" name="inside">
            <el-menu :default-active="insideActive" mode="horizontal" @select="handleSelect">
              <el-menu-item :disabled="true">
                <img src="@/assets/image/icons/fileIcons/seal.png" />组织维度
              </el-menu-item>
            ...
            <el-row class="middleSearch">
              ...
              <el-col :span="10">
                ...
                <el-switch v-model="empower" active-text="分项" inactive-text="统一"></el-switch>
              </el-col>
            </el-row>
            <div class="treeContent">11</div>
            <el-menu mode="horizontal">
              <el-menu-item :disabled="true">
                <img src="@/assets/image/icons/fileIcons/power.png" />云端权限
              </el-menu-item>
              <el-menu-item>
                <el-checkbox
                  :indeterminate="isIndeterminate"
                  v-model="checkAllLimit"
                  @change="handleAllLimitChange"
                >所有</el-checkbox>
              </el-menu-item>
            </el-menu>
...
export default {
  ...
  mounted(){
    this.folderRight = shareCloud.folderRight;
    this.blankRight = shareCloud.blankRight;
  }
}
